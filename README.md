<div align="left">
  ...
  <p>
    ...
    <div>◎ 리스트 파라미터가 전달되어 select문이 여러번 수행되어 결과가 합산되는 로직에서 db서버와의 세션이 반복 생성되지 않도록 하나의 트랜잭션으로 처리 필요 ▶<i>[완료여부: X]</i> </div>
    <div>◎ 운영전환 시 동일한 어댑터에 속한 인터페이스이더라도 순차전환 할 수 있도록 인터페이스 활성화/비활성화 기능 필요. → 인터페이스 활성/비활성 HTTP API 어댑터 생성 후 개별적인 어댑터에서는 스케줄링을 하면서 API 어댑터와 파일로 인터페이스 하는 방식. ▶<i>[완료여부: X]</i> </div>
  </p>
</div>
